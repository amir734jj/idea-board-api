...
namespace Logic.Logic
{
    public class BoardLogic : IBoardLogic
    {
        ...
        public async Task<Project> AddComment(int projectId, User user, CommentViewModel commentViewModel)
        {
            ...
            await _notificationLogic.AddNotification(user, new UserNotification
            {
                Subject = "New Comment",
        ...
        public async Task<Project> Vote(int projectId, User user, Vote vote)
        {
            ...
            {
                var previousVote = usr.Votes.FirstOrDefault(y => y.Project.Id == projectId);

                if (previousVote == null)
                {
                    isNewVote = true;
                    
                    usr.Votes.Add(new UserVote { ProjectId = projectId, UserId = usr.Id, Value = vote });
                }
                else
                {
                    if (previousVote.Value == vote)
                    {
                        usr.Votes.Remove(previousVote);
                    }
                    else
                    {
